...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++){
    let power = i + 1;
    let powerNum = Math.pow(arr[i], power);
...
const mapTwoToThe = (arr) => {
  // Solution code here...
   // Map sort of worked but the problem calls for a for loop
  // let powerMap = arr.map (num => Math.pow(num, 2));
  // return powerMap;
};
