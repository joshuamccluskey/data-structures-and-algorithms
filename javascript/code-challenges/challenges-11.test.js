...
const transformToLis = (obj) => {
  let resultsArr = [];
  let objectKeysArr = Object.keys(obj);
  let objectValsArr = Object.values(obj);

  for (let i = 0; i < objectKeysArr.length; i++) {
    resultsArr.push(`<li>${objectKeysArr[i]}: ${objectValsArr[i]}</li>`);
  }
  return resultsArr;
...
const totalSum = (input) => input.reduce((sum, el) => sum.concat(el)).reduce((sum, el) => sum + el);
// let sumOne = input.map(arr => arr.reduce((sum, item) => sum += item, 0));
// let sumTwo = sumOne.map(arr => arr.reduce((sum, item) => sum += item, 0));
// return sumTwo;
